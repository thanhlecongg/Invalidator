...
_MODEL_PATH = "model/model_ods_nogt.joblib"
...
def do_train():
    ...
    train_data = np.array(bug_patch_diffs)[idxs[n_val:]]
    val_data = np.array(bug_patch_diffs)[idxs[:n_val]]
...
def do_eval(threshold):
    ...
    test_data = np.array(bug_patch_diffs_t)
