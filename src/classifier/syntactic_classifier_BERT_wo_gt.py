...
_MODEL_PATH = "model/model_bert_wogt.joblib"
...
def get_features(buggy, patched):
    ...
    fe = np.hstack((subtract1, multiple1, cos1, euc1))
...
def do_train():
    ...
    train_data = get_features(buggy, patched)[idxs[n_val:]]
    val_data = get_features(buggy, patched)[idxs[:n_val]]
    train(train_data=train_data, labels=train_label, val_data=val_data, labels_val=val_label)

def do_eval(threshold):
    #Loading data
    with open(_TRAIN_DATA, 'rb') as input:
        data = pickle.load(input)
